 ...
 */
public class AliasCmd extends AbstractJavaShellCommand {
    ...
    public void exec(String[] args, JShellCommandContext context) {
        ...
        if (commandArgsCount == 0) {
            for (String k : context.getGlobalContext().aliases().getAll()) {
                context.out().print(k);
                context.out().print(" = ");
                context.out().print(context.getGlobalContext().aliases().get(k));
                context.out().print("\n");
            }
        } else {
            for (int i = 0; i < args.length; i++) {
                ...
                if (p > 0) {
                    context.getGlobalContext().aliases().set(args[i].substring(0, p), args[i].substring(p + 1));
                } else {
                    throw new JShellCmdSyntaxError(1,
                            args,
                            getName(),
                            getHelp(),
