 ...
 */
public class PropsCmd extends AbstractJavaShellCommand {
    ...
    public void exec(String[] args, JShellCommandContext context) {
        if (args.length == 0) {
            for (Map.Entry e : System.getProperties().entrySet()) {
                context.out().println(e.getKey() + " = " + ShellUtils.escapeString(String.valueOf(e.getValue())));
            }
        } else {
            throw new JShellCmdSyntaxError(1, args, getName(), getHelp(), "Wrong number of arguments " + args.length + " != 0");
        }
    }
