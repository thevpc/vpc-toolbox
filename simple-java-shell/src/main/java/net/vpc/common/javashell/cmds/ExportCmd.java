 ...
 * Time: 23:08:51
 */
public class ExportCmd extends AbstractJavaShellCommand {
    ...
    public void exec(String[] args, JShellCommandContext context) {
        ...
        for (int i = 0; i < commandArgsCount; i++) {
            ...
            if(name.contains("=")){
                int x = name.indexOf('=');
                String v = name.substring(0, x);
                context.vars().set(v,name.substring(x+1));
                context.vars().export(v);
            }else{
                context.vars().export(name);
            }
        }
    }
